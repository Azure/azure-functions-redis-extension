...
> 
> 3.  Use `AzureRedisCacheExtension.sln` under the `azurerediscache-demo-function` folder to open the solution. For demo purposes the Azure function trigger in the `azurerediscache-demo-function` folder already has the `azure-functions-azurerediscache-extension` library linked as a _project reference_.
